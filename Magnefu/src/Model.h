...
namespace Magnefu
{
	...
	public:
		...
		void Init(Shader* shader, Cache& textureCache, Cache& materialCache);
		std::string m_Filepath;

		std::vector<std::unique_ptr<Mesh>> m_Meshes;

		std::vector<MaterialData>            m_MaterialList;
		void Draw(Shader* shader, Cache& textureCache, Cache& materialCache);
		void OnImGUIRender();
	...
	private:
		...
		void SetShaderUniforms(Shader* shader, Cache& materialCache);
