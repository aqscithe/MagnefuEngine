...
namespace Magnefu
{
	...
	void Shader::CreateGraphicsPipeline(const ShaderDesc& desc)
	{
		...
		BindGroup& renderpassGlobals = static_cast<BindGroup&>(rm.GetBindGroup(desc.BindGroups[0]));
		BindGroup& material = static_cast<BindGroup&>(rm.GetBindGroup(desc.BindGroups[1]));
	...
	ShaderList Shader::ParseShader(const char* filepath)
	{
	...
	VkShaderModule Shader::CreateShaderModule(const ShaderSource& source)
	{
	...
	VkDynamicState Shader::TranslateDynamicState(const DynamicState& state)
	{
		switch (state)
		{
	...
	VkPolygonMode Shader::TranslatePolygonMode(const PolygonMode& mode)
	{
	...
	VkCullModeFlags Shader::TranslateCullMode(const CullMode& mode)
	{
		switch (mode)
		{
	...
	VkCompareOp Shader::TranslateDepthCompareOp(const DepthCompareOp& compareOp)
	{
		switch (compareOp)
		{
