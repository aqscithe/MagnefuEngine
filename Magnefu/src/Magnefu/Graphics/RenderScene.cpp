...
namespace Magnefu {
    ...
    static FrameGraphResource* get_output_texture(FrameGraph* frame_graph, FrameGraphResourceHandle input) {
        ...
        MF_CORE_ASSERT((output_resource != nullptr), "");
    ...
    void PhysicsVertex::add_joint(u32 vertex_index) {
        ...
        MF_CORE_ASSERT((joint_count < k_max_joint_count), "");
    ...
    void DepthPrePass::free_gpu_resources(GraphicsContext& gpu) {
        if (!enabled)
            return;
    ...
    void DepthPyramidPass::on_resize(GraphicsContext& gpu, FrameGraph* frame_graph, u32 new_width, u32 new_height) {
    ...
    void DepthPyramidPass::free_gpu_resources(GraphicsContext& gpu) {
        if (!enabled)
            return;
    ...
    void GBufferPass::free_gpu_resources(GraphicsContext& gpu) {
        if (!enabled)
            return;
    ...
    void LateGBufferPass::free_gpu_resources(GraphicsContext& gpu) {
        if (!enabled)
            return;
    ...
    void LightPass::on_resize(GraphicsContext& gpu, FrameGraph* frame_graph, u32 new_width, u32 new_height) {
        if (!enabled)
            return;
    ...
    void LightPass::free_gpu_resources(GraphicsContext& gpu) {
        if (!enabled)
            return;
    ...
    void TransparentPass::free_gpu_resources(GraphicsContext& gpu) {
        if (!enabled)
            return;
    ...
    static void load_debug_mesh(cstring filename, Allocator* resident_allocator, Renderer* renderer, u32& index_count, BufferResource** mesh_buffer, BufferResource** index_buffer) {
        ...
        Array<u32> indices;
        indices.init(resident_allocator, mfkilo(64));
        ...
        for (u32 mesh_index = 0; mesh_index < mesh_scene->mNumMeshes; ++mesh_index) {
            ...
            MF_CORE_ASSERT(((mesh->mPrimitiveTypes & aiPrimitiveType_TRIANGLE) != 0), "");
            ...
            for (u32 face_index = 0; face_index < mesh->mNumFaces; ++face_index) {
                MF_CORE_ASSERT((mesh->mFaces[face_index].mNumIndices == 3), "");
    ...
    void DebugPass::prepare_draws(RenderScene& scene, FrameGraph* frame_graph, Allocator* resident_allocator, StackAllocator* scratch_allocator) {
        ...
        sizet marker = scratch_allocator->getMarker();
        ...
        filename = mesh_name.append_use_f("%s/cone.obj", MAGNEFU_MODEL_FOLDER);
        ...
        scratch_allocator->freeToMarker(marker);
    ...
    void DebugPass::free_gpu_resources(GraphicsContext& gpu) {
        if (!enabled)
            return;
    ...
    void DoFPass::on_resize(GraphicsContext& gpu, FrameGraph* frame_graph, u32 new_width, u32 new_height) {
        if (!enabled) return;
    ...
    void DoFPass::prepare_draws(RenderScene& scene, FrameGraph* frame_graph, Allocator* resident_allocator, StackAllocator* scratch_allocator) {
        ...
        MF_CORE_ASSERT((depth_texture != nullptr), "");
    ...
    void DoFPass::free_gpu_resources(GraphicsContext& gpu) {
        if (!enabled)
            return;
    ...
    void CullingEarlyPass::render(u32 current_frame_index, CommandBuffer* gpu_commands, RenderScene* render_scene) {
        ...
        u32 group_x = Magnefu::ceilu32(render_scene->mesh_instances.size / 64.0f);
    ...
    void CullingEarlyPass::free_gpu_resources(GraphicsContext& gpu) {
    ...
    void CullingLatePass::free_gpu_resources(GraphicsContext& gpu) {
    ...
    void RayTracingTestPass::on_resize(GraphicsContext& gpu, FrameGraph* frame_graph, u32 new_width, u32 new_height) {
        if (!enabled) {
            return;
        }
    ...
    void RayTracingTestPass::free_gpu_resources(GraphicsContext& gpu) {
        if (!enabled) {
            return;
        }
    ...
    void ShadowVisibilityPass::prepare_draws(RenderScene& scene, FrameGraph* frame_graph, Allocator* resident_allocator, StackAllocator* scratch_allocator) {
        ...
        MF_CORE_ASSERT((resource != nullptr), "");
    ...
    void PointlightShadowPass::pre_render(u32 current_frame_index, CommandBuffer* gpu_commands, FrameGraph* frame_graph, RenderScene* render_scene) {
        ...
        u32 group_x = Magnefu::ceilu32(render_scene->mesh_instances.size * render_scene->active_lights / 32.0f);
        gpu_commands->dispatch(group_x, 1, 1);
        ...
        group_x = Magnefu::ceilu32(render_scene->active_lights / 32.0f);
        gpu_commands->dispatch(group_x, 1, 1);
        ...
        const u32 tile_x_count = Magnefu::ceilu32(render_scene->scene_data.resolution_x / tile_size);
        const u32 tile_y_count = Magnefu::ceilu32(render_scene->scene_data.resolution_y / tile_size);
    ...
    void PointlightShadowPass::recreate_lightcount_dependent_resources(RenderScene& scene) {
        ...
        Magnefu::TextureCreation texture_creation;
        ...
        if (shadow_maps_pool.index == k_invalid_index) {
            shadow_maps_pool = gpu.allocate_texture_pool(cubemap_shadow_array_texture, mfgiga(1));
        }
        ...
        // Create framebuffer
        Magnefu::FramebufferCreation frame_buffer_creation;
    ...
    void PointlightShadowPass::update_dependent_resources(GraphicsContext& gpu, FrameGraph* frame_graph, RenderScene* render_scene) {
    }
    ...
    void VolumetricFogPass::on_resize(GraphicsContext& gpu, FrameGraph* frame_graph, u32 new_width, u32 new_height) {
        if (!enabled)
            return;
    ...
    void VolumetricFogPass::prepare_draws(RenderScene& scene, FrameGraph* frame_graph, Allocator* resident_allocator, StackAllocator* scratch_allocator) {
        ...
        texture_creation.reset().set_size(scene.volumetric_fog_tile_count_x, scene.volumetric_fog_tile_count_y, scene.volumetric_fog_slices)
            .set_format_type(VK_FORMAT_R16G16B16A16_SFLOAT, TextureType::Texture3D).set_flags(Magnefu::TextureFlags::Compute_mask).set_name("froxel_data_texture_0");
        ...
        texture_creation.reset().set_size(64, 64, 64).set_format_type(VK_FORMAT_R8_UNORM, TextureType::Texture3D)
            .set_flags(Magnefu::TextureFlags::Compute_mask).set_name("volumetric_noise");
    ...
    void VolumetricFogPass::free_gpu_resources(GraphicsContext& gpu) {
    ...
    void VolumetricFogPass::update_dependent_resources(GraphicsContext& gpu, FrameGraph* frame_graph, RenderScene* render_scene) {
        if (!enabled)
            return;
    ...
    void TemporalAntiAliasingPass::on_resize(GraphicsContext& gpu, FrameGraph* frame_graph, u32 new_width, u32 new_height) {
    ...
    void TemporalAntiAliasingPass::free_gpu_resources(GraphicsContext& gpu) {
    ...
    void TemporalAntiAliasingPass::update_dependent_resources(GraphicsContext& gpu, FrameGraph* frame_graph, RenderScene* render_scene) {
    }
    ...
    void MotionVectorPass::on_resize(GraphicsContext& gpu, FrameGraph* frame_graph, u32 new_width, u32 new_height) {
        if (!enabled)
            return;
    }
    ...
    void MotionVectorPass::free_gpu_resources(GraphicsContext& gpu) {
        if (!enabled)
            return;
    ...
    void MotionVectorPass::update_dependent_resources(GraphicsContext& gpu, FrameGraph* frame_graph, RenderScene* render_scene) {
        if (!enabled)
            return;
    ...
    void RenderScene::update_animations(f32 delta_time) {
        ...
        for (u32 ac = 0; ac < animation.channels.size; ++ac) {
            ...
            if (sampler.interpolation_type != AnimationSampler::Linear) {
                MF_CORE_WARN("Interpolation {} still not supported.", sampler.interpolation_type);
                continue;
            }
            ...
            for (u32 ki = 0; ki < sampler.key_frames.size - 1; ++ki) {
                ...
                if (current_time >= keyframe && current_time <= next_keyframe) {
                    ...
                    MF_CORE_ASSERT((channel.target_node < 256), "");
    ...
    void RenderScene::upload_gpu_data(UploadGpuDataContext& context) {
        ...
        sizet current_marker = context.scratch_allocator->getMarker();
        ...
        for (u32 i = 0; i < active_lights; ++i) {
            ...
            const u32 min_bin = Magnefu::max(0, Magnefu::floori32(light.projected_z_min * k_light_z_bins));
            const u32 max_bin = Magnefu::max(0, Magnefu::ceili32(light.projected_z_max * k_light_z_bins));
