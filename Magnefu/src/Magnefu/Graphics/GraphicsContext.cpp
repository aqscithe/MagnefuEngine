...
namespace Magnefu
{
		...
		m_WindowHandle(windowHandle), m_VkInstance(VkInstance()), m_VkPhysicalDevice(VK_NULL_HANDLE)
	{
		...
		MF_CORE_ASSERT(!s_Instance, "GraphicsContext instance already exists.");
	...
	GraphicsContext::~GraphicsContext()
	{
		...
		// -- Deallocate GraphicsContext Arrays ------------------------------- //
	...
	void GraphicsContext::Init()
	{
		...
		bufferThread = std::thread(&GraphicsContext::AllocateBufferMemory, this);

	}

	void GraphicsContext::TempSecondaryInit()
	{
		// -- FrameBuffer Class -- //
		CreateImageViews();
	...
	void GraphicsContext::OnImGuiRender()
	{

	}

	void GraphicsContext::OnFinish()
	{
		vkDeviceWaitIdle(m_VkDevice);

	}

	std::any GraphicsContext::GetContextInfo(const std::string& name)
	{
		...
		else if (name == "SwapChainImages") return m_SwapChainImages.raw();
	...
	MemoryStats GraphicsContext::GetMemoryStats()
	{
	...
	void GraphicsContext::CreateVkInstance()
	{
		...
		instanceCreateInfo.enabledExtensionCount = static_cast<uint32_t>(extensions.count());
		instanceCreateInfo.ppEnabledExtensionNames = extensions.raw();
	...
	void GraphicsContext::SetupValidationLayers(bool& allLayersAvailable)
	{
#ifdef MF_DEBUG
		// Print extensions
	...
	void GraphicsContext::CreateWindowSurface()
	{
	...
	void GraphicsContext::CreateLogicalDevice()
	{
		...
		logicalDevCreateInfo.pQueueCreateInfos = queueCreateInfos.raw();
		logicalDevCreateInfo.queueCreateInfoCount = static_cast<uint32_t>(queueCreateInfos.count());
	...
	void GraphicsContext::CreateVmaAllocator()
	{
		...
		*/
		...
		outInfo.pHeapSizeLimit = heapSizeLimit.raw();
		*/
	...
	void GraphicsContext::AllocateBufferMemory()
	{
	...
	//void GraphicsContext::AllocateIndexBuffers(const uint32_t& sceneObjCount, Array<Magnefu::SceneObject>& sceneObjs, VkCommandPool commandPool)
	//{
	//	
	...
	//void GraphicsContext::AllocateVertexBuffers(const uint32_t& sceneObjCount, Array<Magnefu::SceneObject>& sceneObjs, VkCommandPool commandPool)
	//{
	...
	//void GraphicsContext::AllocateUniformBuffers(const uint32_t& sceneObjCount)
	//{
	//	VkDeviceSize totalSize = 0;
	...
	void GraphicsContext::CreateSwapChain()
	{
		...
		m_SwapChainImages.set_size(imageCount);
		vkGetSwapchainImagesKHR(m_VkDevice, m_SwapChain, &imageCount, m_SwapChainImages.raw());
	...
	{
		m_SwapChainImageViews.set_size(m_SwapChainImages.count());
	...
	{
		m_SwapChainFramebuffers.set_size(m_SwapChainImageViews.count());
	...
	void GraphicsContext::CreateColorResources()
	{
	...
	//void GraphicsContext::LoadModels()
	//{
	...
	//		threads[i] = std::thread(&GraphicsContext::LoadSingleModel, this, RESOURCE_PATHS[i], i, ModelType::MODEL_DEFAULT);
	...
	//void GraphicsContext::LoadTextures()
	//{
	//	MF_CORE_ASSERT(
	...
	//		for (int j = 0; j < 3; j++)
	//		{
	//			threads[i * 3 + j] = std::thread(&GraphicsContext::LoadSingleTexture, this, TEXTURE_PATHS[i].ModelIndex, TEXTURE_PATHS[i].Paths[j], j);
	//		}
	//		
	//	}
	...
	//void GraphicsContext::LoadSingleModel(const ResourceInfo& resourceInfo, size_t objIndex, ModelType modelType)
	//{
	//	Application& app = Application::Get();
	//	
	...
	//void GraphicsContext::LoadSingleTexture(int sceneObjIndex, const char* texturePath, int textureType)
	//{
	...
	{
		m_CommandBuffers.set_size(MAX_FRAMES_IN_FLIGHT);

		VkCommandBufferAllocateInfo allocInfo{};
		allocInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
		allocInfo.commandPool = m_CommandPool;
		allocInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
		allocInfo.commandBufferCount = (uint32_t)m_CommandBuffers.count();

		if (vkAllocateCommandBuffers(m_VkDevice, &allocInfo, m_CommandBuffers.raw()) != VK_SUCCESS)
			MF_CORE_ASSERT(false, "failed to allocate command buffers!");

		m_ImGuiCommandBuffers.set_size(MAX_FRAMES_IN_FLIGHT);

		allocInfo = {};
		allocInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
		allocInfo.commandPool = m_CommandPool;
		allocInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
		allocInfo.commandBufferCount = (uint32_t)m_ImGuiCommandBuffers.count();

		if (vkAllocateCommandBuffers(m_VkDevice, &allocInfo, m_ImGuiCommandBuffers.raw()) != VK_SUCCESS)
			MF_CORE_ASSERT(false, "failed to allocate command buffers!");
	...
	Array<const char*> GraphicsContext::GetRequiredExtensions()
	{
	...
	QueueFamilyIndices GraphicsContext::FindQueueFamilies(VkPhysicalDevice device)
	{
	...
	SwapChainSupportDetails GraphicsContext::QuerySwapChainSupport(VkPhysicalDevice device)
	{
		...
		if (formatCount != 0)
		{
			vkGetPhysicalDeviceSurfaceFormatsKHR(device, m_WindowSurface, &formatCount, details.Formats.raw());
		}
		...
		{
			vkGetPhysicalDeviceSurfacePresentModesKHR(device, m_WindowSurface, &presentModeCount, details.PresentModes.raw());
		}
	...
	VkSurfaceFormatKHR GraphicsContext::ChooseSwapSurfaceFormat(const Array<VkSurfaceFormatKHR>& availableFormats)
	{
	...
	VkPresentModeKHR GraphicsContext::ChooseSwapPresentMode(const Array<VkPresentModeKHR>& availablePresentModes)
	{
	...
	void GraphicsContext::CreateSyncObjects()
	{
		...
		m_ImageAvailableSemaphores.set_size(MAX_FRAMES_IN_FLIGHT);
		m_ImGuiRenderFinishedSemaphores.set_size(MAX_FRAMES_IN_FLIGHT);
		m_RenderFinishedSemaphores.set_size(MAX_FRAMES_IN_FLIGHT);

		m_InFlightFences.set_size(MAX_FRAMES_IN_FLIGHT);
		m_ImGuiInFlightFences.set_size(MAX_FRAMES_IN_FLIGHT);
	...
	VkFormat GraphicsContext::FindSupportedFormat(const VkFormat* candidates, u32 candidateCount, VkImageTiling tiling, VkFormatFeatureFlags features)
	{
		for (int format = 0; format < candidateCount; format++)
		{
			VkFormatProperties props;
			vkGetPhysicalDeviceFormatProperties(m_VkPhysicalDevice, candidates[format], &props);
	...
	VkSampleCountFlagBits GraphicsContext::GetMaxUsableSampleCount()
	{
	...
	void GraphicsContext::PerformGraphicsOps()
	{
		// GRAPHICS SUBMISSION //
