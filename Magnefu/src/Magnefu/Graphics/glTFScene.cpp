...
namespace Magnefu {
    ...
    void glTFScene::fill_pbr_material(glTF::glTF& gltf_scene, Renderer& renderer, glTF::Material& material, PBRMaterial& pbr_material) {
        ...
        if (material.emissive_factor_count != 0) {
            MF_CORE_ASSERT((material.emissive_factor_count == 3), "");
    ...
    u16 glTFScene::get_material_texture(GraphicsContext& gpu, glTF::glTF& gltf_scene, glTF::TextureInfo* texture_info) {
    ...
    u16 glTFScene::get_material_texture(GraphicsContext& gpu, glTF::glTF& gltf_scene, i32 gltf_texture_index) {
    ...
    void glTFScene::add_mesh(cstring filename, cstring path, StackAllocator* temp_allocator, AsynchronousLoader* async_loader) {
        ...
        sizet temp_marker = temp_allocator->getMarker();
        ...
        for (u32 mi = 0; mi < gltf_scene.meshes_count; ++mi) {
            ...
            for (u32 p = 0; p < mesh.primitives_count; ++p) {
                ...
                f32 radius = Magnefu::max(glms_vec3_distance(position_max, bounding_center), glms_vec3_distance(position_min, bounding_center));
                ...
                temp_allocator->freeToMarker(temp_marker);
        ...
        while (nodes_to_visit.size) {
            ...
            for (u32 primitive_index = 0; primitive_index < gltf_mesh.primitives_count; ++primitive_index) {
                ...
                if (node.skin != glTF::INVALID_INT_VALUE) {
                    MF_CORE_ASSERT((node.skin < skins.size), "");
                ...
                scene_graph->set_mesh_data(node_index, mesh_instances.count() - 1);
            }
        }
        ...
        for (u32 mesh_index = 0; mesh_index < transform_count; ++mesh_index) {
            ...
            MF_CORE_ASSERT((mesh_instance.mesh != nullptr), "");
        ...
        for (u32 animation_index = 0; animation_index < gltf_scene.animations_count; ++animation_index) {
            ...
            for (u32 channel_index = 0; channel_index < gltf_animation.channels_count; ++channel_index) {
                ...
                channel.target_type = (Magnefu::AnimationChannel::TargetType)gltf_channel.target_type;
            }
            ...
            for (u32 sampler_index = 0; sampler_index < gltf_animation.samplers_count; ++sampler_index) {
                ...
                sampler.interpolation_type = (Magnefu::AnimationSampler::Interpolation)gltf_sampler.interpolation;
                ...
                // TODO: (leon) fix animation copying
                // issue with memory allocation to sampler.data
                ...
                // Copy animation data
                {
                    ...
                    MF_CORE_ASSERT((buffer_accessor.count == key_frames_count), "");

                    u8* buffer_data = (u8*)buffers_data[buffer_view.buffer] + byte_offset;

                    sampler.data = (vec4s*)mfallocaa(sizeof(vec4s) * buffer_accessor.count, resident_allocator, 16);
        ...
        for (u32 si = 0; si < gltf_scene.skins_count; ++si) {
            ...
            memoryCopy(skin.joints.data, gltf_skin.joints, sizeof(i32) * gltf_skin.joints_count);
            ...
            MF_CORE_ASSERT((buffer_accessor.count == skin.joints.size), "");
            skin.inverse_bind_matrices = (mat4s*)mfallocaa(sizeof(mat4s) * buffer_accessor.count, resident_allocator, 16);

            u8* buffer_data = (u8*)buffers_data[buffer_view.buffer] + byte_offset;
            memoryCopy(skin.inverse_bind_matrices, buffer_data, sizeof(mat4s) * buffer_accessor.count);
    ...
    void glTFScene::shutdown(Renderer* renderer) {
        ...
        for (u32 ai = 0; ai < animations.size; ++ai) {
            ...
            for (u32 si = 0; si < animation.samplers.size; ++si) {
                ...
                mffree(sampler.data, resident_allocator);
            }
        ...
        for (u32 si = 0; si < skins.size; ++si) {
            ...
            mffree(skin.inverse_bind_matrices, resident_allocator);
