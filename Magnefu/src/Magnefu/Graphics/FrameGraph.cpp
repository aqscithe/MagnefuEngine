...
namespace Magnefu
{
    ...
    void FrameGraph::shutdown() {
        for (u32 i = 0; i < nodes.size; ++i) {
            ...
            builder->gpu->destroy_framebuffer(node->framebuffer);
    ...
    void FrameGraph::parse(cstring file_path, StackAllocator* temp_allocator) {
        ...
        sizet current_allocator_marker = temp_allocator->getMarker();
        ...
        for (sizet i = 0; i < passes.size(); ++i) {
            ...
            for (sizet oi = 0; oi < pass_outputs.size(); ++oi) {
                ...
                MF_CORE_ASSERT(!output_type.empty(), "");
            ...
            MF_CORE_ASSERT(!name_value.empty(), "");
        ...
        temp_allocator->freeToMarker(current_allocator_marker);
    }
    ...
    static void compute_edges(FrameGraph* frame_graph, FrameGraphNode* node, u32 node_index) {
        for (u32 r = 0; r < node->inputs.size; ++r) {
            ...
            if (output_resource == nullptr && !resource->resource_info.external) {
                ...
                MF_CORE_ASSERT(false, "Requested resource is not produced by any node and is not external.");
                continue;
            }
    ...
    static void create_render_pass(FrameGraph* frame_graph, FrameGraphNode* node) {
        ...
        node->render_pass = frame_graph->builder->gpu->create_render_pass(render_pass_creation);
    }
    ...
    void FrameGraph::compile() {
        ...
        MF_CORE_ASSERT((sorted_nodes.size == nodes.size), "");
                ...
                if (!resource->resource_info.external && allocations[resource_index].index == k_invalid_index) {
                    ...
                    if (resource->type == FrameGraphResourceType_Attachment) {
                        ...
                        if (free_list.size > 0) {
                            ...
                            TextureHandle handle = builder->gpu->create_texture(texture_creation);
                    ...
                    MF_CORE_DEBUG("Output {} allocated on node {}", resource->name, nodes[i].index);
                }
            }
        ...
        for (u32 n = 0; n < nodes.size; ++n) {
            ...
            for (u32 i = 0; i < node->inputs.size; ++i) {
                ...
                if (resource->type == FrameGraphResourceType_Texture) {
                    Texture* texture = gpu_commands->gpu->access_texture(resource->resource_info.texture.texture);
    ...
    void FrameGraphResourceCache::init(Allocator* allocator, GraphicsContext* device_)
    {
        device = device_;
    ...
    void FrameGraphNodeCache::init(Allocator* allocator, GraphicsContext* device_)
    {
        device = device_;
    ...
    void FrameGraphBuilder::init(GraphicsContext* gpu_) {
        ...
        resource_cache.init(allocator, gpu);
        node_cache.init(allocator, gpu);
