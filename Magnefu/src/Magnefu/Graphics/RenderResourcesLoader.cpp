...
#include "RenderResourcesLoader.hpp"
...
namespace Magnefu 
{
    ...
    // Utility methods ////////////////////////////////////////////////////////
    static void             shader_concatenate(cstring filename, Magnefu::StringBuffer& path_buffer, Magnefu::StringBuffer& shader_buffer, Magnefu::Allocator* temp_allocator);
    static VkBlendFactor    get_blend_factor(const std::string factor);
    static VkBlendOp        get_blend_op(const std::string op);
    static void             parse_gpu_pipeline(nlohmann::json& pipeline, Magnefu::PipelineCreation& pc, Magnefu::StringBuffer& path_buffer,
    ...
    GpuTechnique* RenderResourcesLoader::load_gpu_technique(cstring json_path) {
        ...
        shader_code_buffer.init(mfkilo(256), temp_allocator);
    ...
    TextureResource* RenderResourcesLoader::load_texture(cstring path, bool generate_mipmaps) {
        ...
        if (!image_data) {
            MF_CORE_ERROR("Error loading texture {}", path);
            return nullptr;
        }
    ...
    void shader_concatenate(cstring filename, Magnefu::StringBuffer& path_buffer, Magnefu::StringBuffer& shader_buffer, Magnefu::Allocator* temp_allocator) {
        using namespace Magnefu;

        // Read file and concatenate it
        path_buffer.clear();
        cstring shader_path = path_buffer.append_use_f("%s%s", MAGNEFU_SHADER_FOLDER, filename);
        ...
        if (shader_read_result.data) {
            ...
            MF_CORE_ERROR("Cannot read file {}", shader_path);
        }
    }

    void parse_gpu_pipeline(nlohmann::json& pipeline, Magnefu::PipelineCreation& pc, Magnefu::StringBuffer& path_buffer,
        Magnefu::StringBuffer& shader_buffer, Magnefu::Allocator* temp_allocator, Magnefu::Renderer* renderer,
        Magnefu::FrameGraph* frame_graph, Magnefu::StringBuffer& pass_name_buffer) {
        using json = nlohmann::json;
        using namespace Magnefu;
        ...
        if (render_pass.is_string()) {
            ...
            if (node) {
                ...
                MF_CORE_WARN("Cannot find render pass {}. Defaulting to swapchain", name.c_str());
                pc.render_pass = renderer->gpu->get_swapchain_output();
            }
        }
    }
