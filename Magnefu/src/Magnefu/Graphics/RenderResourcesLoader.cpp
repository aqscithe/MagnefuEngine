...
{
	// -- Utility methods ------------------------------------------------------------- //
	static void             shader_concatenate(cstring filename, StringBuffer& path_buffer, StringBuffer& shader_buffer, Allocator* temp_allocator);
	...
	void RenderResourcesLoader::init(Renderer* renderer_, StackAllocator* temp_allocator_, FrameGraph* frame_graph_)
	{
		renderer = renderer_;
	...
	void RenderResourcesLoader::load_gpu_technique(cstring json_path)
	{
		...
		shader_code_buffer.init(mfkilo(64), temp_allocator);
	...
	void parse_gpu_pipeline(nlohmann::json& pipeline, PipelineCreation& pc, StringBuffer& path_buffer, StringBuffer& shader_buffer, Allocator* temp_allocator, Renderer* renderer, FrameGraph* frame_graph)
	{
		using json = nlohmann::json;
		...
		if (render_pass.is_string()) {
			...
			{
				MF_CORE_ERROR("Cannot find render pass {}. Defaulting to swapchain", name.c_str());
				pc.render_pass = renderer->gpu->get_swapchain_output();
			}
		}
	}
