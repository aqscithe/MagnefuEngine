...
namespace Magnefu {
    ...
    // Utility methods ////////////////////////////////////////////////////////
    static u64              shader_concatenate(cstring filename, Magnefu::StringBuffer& path_buffer, Magnefu::StringBuffer& shader_buffer, Magnefu::Allocator* temp_allocator);
    static VkBlendFactor    get_blend_factor(const std::string factor);
    static VkBlendOp        get_blend_op(const std::string op);
    static bool             parse_gpu_pipeline(nlohmann::json& pipeline, Magnefu::PipelineCreation& pc, Magnefu::StringBuffer& path_buffer,
                ...
                if (vertex_attributes.is_array()) {
                    ...
                    for (sizet v = 0; v < vertex_attributes.size(); ++v) {
                        ...
                        if (attribute_format.is_string()) {
                            ...
                            MF_CORE_ASSERT((vertex_attribute.format != VertexComponentFormat::Count), "");
                        }
    ...
    u64 shader_concatenate(cstring filename, Magnefu::StringBuffer& path_buffer, Magnefu::StringBuffer& shader_buffer, Magnefu::Allocator* temp_allocator) {
        ...
        if (shader_read_result.data) {
            ...
            hashed_memory = Magnefu::hash_bytes(shader_read_result.data, strlen(shader_read_result.data));
        }
        else {
    ...
    bool parse_gpu_pipeline(nlohmann::json& pipeline, Magnefu::PipelineCreation& pc, Magnefu::StringBuffer& path_buffer,
        Magnefu::StringBuffer& shader_buffer, Magnefu::Allocator* temp_allocator, Magnefu::Renderer* renderer,
        Magnefu::FrameGraph* frame_graph, Magnefu::StringBuffer& pass_name_buffer,
        const Array<VertexInputCreation>& vertex_input_creations, FlatHashMap<u64, u16>& name_to_vertex_inputs,
        cstring technique_name, bool use_cache, bool parent_technique) {
