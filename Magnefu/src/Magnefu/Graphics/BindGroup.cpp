...
namespace Magnefu
{
	...
	void BindGroup::CreateDescriptorSetLayout(const BindingLayout& layout)
	{
	...
	void BindGroup::CreateBindingBuffers(const BindingBufferDescs& descriptions)
	{
		m_Uniforms = Application::Get().GetResourceManager().CreateBuffer(descriptions.Uniforms);
	}

	void BindGroup::CreateBindingTextures(const BindingTextureDescs& descriptions)
	{
	...
	void BindGroup::CreateDescriptorPool(const BindingLayoutType& type, bool IsTextured)
	{
		switch (type)
		{
		case BindingLayoutType::LAYOUT_MATERIAL_DEFAULT:
	...
	void BindGroup::CreateDescriptorSets(const BindingLayoutType& type, bool isTextured)
	{
		Application& app = Application::Get();
		ResourceManager& rm = app.GetResourceManager();
		VkDevice device = VulkanContext::Get().GetDevice();
		UniformBuffer& uniformBuffer = rm.GetBuffer(m_Uniforms);
		...
		case BindingLayoutType::LAYOUT_MATERIAL_DEFAULT:
		{
			...
			for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
			{
				...
				if (isTextured)
				{
					...
					imageInfo[0].imageView = rm.GetTexture(m_DiffuseTexture).GetImageView();
					imageInfo[0].sampler = rm.GetTexture(m_DiffuseTexture).GetSampler();

					// ARM
					imageInfo[1].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
					imageInfo[1].imageView = rm.GetTexture(m_ARMTexture).GetImageView();
					imageInfo[1].sampler = rm.GetTexture(m_ARMTexture).GetSampler();

					// Normal
					imageInfo[2].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
					imageInfo[2].imageView = rm.GetTexture(m_NormalTexture).GetImageView();
					imageInfo[2].sampler = rm.GetTexture(m_NormalTexture).GetSampler();
				}
		...
		case BindingLayoutType::LAYOUT_RENDERPASS:
		{
			...
			for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
			{
				...
				// LTC1
				imageInfo[0].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
				imageInfo[0].imageView = rm.GetTexture(m_LTC1_Texture).GetImageView();
				imageInfo[0].sampler =rm.GetTexture(m_LTC1_Texture).GetSampler();

				// LTC2
				imageInfo[1].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
				imageInfo[1].imageView = rm.GetTexture(m_LTC2_Texture).GetImageView();
				imageInfo[1].sampler = rm.GetTexture(m_LTC2_Texture).GetSampler();
	...
	VkDescriptorType BindGroup::GetDescriptorType(const BindingType& type)
	{
		switch (type)
		{
