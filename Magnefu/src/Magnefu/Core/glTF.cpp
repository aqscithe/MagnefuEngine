...
namespace Magnefu
{
    ...
    static void try_load_type(json& json_data, cstring key, glTF::Accessor::Type& type)
    {
        ...
        if (value == "SCALAR") {
            ...
            MF_CORE_ASSERT(false, "Invalid load type");
        }
    }
        ...
        if (json_array.is_array()) {
            ...
            for (sizet i = 0; i < count; ++i) {
                ...
                if (target_path == "scale") {
                    ...
                    MF_CORE_ASSERT(false, "Error parsing target path {}", target_path.c_str());
    ...
    glTF::glTF gltf_load_file(cstring file_path) {
        ...
        if (!file_exists(file_path)) {
            MF_CORE_ERROR("Error: file {} does not exists.", file_path);
            return result;
        }

        Allocator* heap_allocator = &MemoryService::Instance()->systemAllocator;

        FileReadResult read_result = file_read_text(file_path, heap_allocator);

        json gltf_data = json::parse(read_result.data);

        result.allocator.init(mfmega(2));
...
i32 Magnefu::glTF::get_data_offset(i32 accessor_offset, i32 buffer_view_offset) {
