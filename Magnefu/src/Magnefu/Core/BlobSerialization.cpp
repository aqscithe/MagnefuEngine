...
namespace Magnefu
{
    ...
    void BlobSerializer::write_common(Allocator* allocator_, u32 serializer_version_, sizet size) {
        ...
        // Allocate memory
        blob_memory = (char*)mfalloca(size + sizeof(BlobHeader), allocator_);
        MF_CORE_ASSERT(blob_memory, "Invalid pointer");
    ...
    void BlobSerializer::serialize(cstring data) 
    {
        ...
        MF_CORE_ASSERT(false, "To be implemented!");
    }
    ...
    char* BlobSerializer::allocate_static(sizet size) 
    {
        ...
        {
            MF_CORE_WARN("Blob allocation error: allocated, requested, total - {0} + {1} > {2}", allocated_offset, size, total_size);
            return nullptr;
        }
    ...
    void BlobSerializer::serialize(RelativeString* data) {
        ...
        if (is_reading) {
            ...
            MF_CORE_INFO("Written {}, Found {}", data->c_str(), destination_data);
    ...
    void BlobSerializer::allocate_and_set(RelativeString& string, cstring format, ...) {
        ...
        if (written_chars < 0) 
        {
            MF_CORE_WARN("New string too big for current buffer! Please allocate more size.\n");
        }
