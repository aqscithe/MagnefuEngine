...
#define MF_CORE_ASSERT_OVERFLOW() //MF_CORE_ASSERT(false)
#else
#define MF_CORE_ASSERT_OVERFLOW()
...
namespace Magnefu
{
    ...
    void StringBuffer::init(sizet size, Allocator* allocator_) 
    {
        ...
        if (size < 1) 
        {
            MF_CORE_ERROR("ERROR: Buffer cannot be empty!");
            return;
        }
        allocator = allocator_;
        data = (char*)mfalloca(size + 1, allocator_);
        MF_CORE_ASSERT(data, "Invalid data ptr");
        data[0] = 0;
    ...
    void StringBuffer::append_f(const char* format, ...) 
    {
        ...
        {
            MF_CORE_ASSERT_OVERFLOW();
            MF_CORE_ERROR("Buffer full! Please allocate more size.");
            return;
        }
        ...
        if (written_chars < 0) {
            MF_CORE_ASSERT_OVERFLOW();
            MF_CORE_ERROR("New string too big for current buffer! Please allocate more size.");
        }
    }
    ...
    void StringArray::shutdown() {
        ...
        mffree(string_to_index, allocator);
    ...
    cstring StringArray::intern(cstring string) 
    {
        ...
        const sizet hashed_string = Magnefu::hash_bytes((void*)string, length, seed);
