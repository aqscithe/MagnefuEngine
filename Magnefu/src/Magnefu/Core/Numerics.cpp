...
namespace Magnefu
{
#if defined (MAGNEFU_MATH_OVERFLOW_CHECK)
...
#define hy_math_convert(val, max, type, func)\
    ...
    if (abs(value_container) > max)\
        MF_CORE_ERROR( "Overflow converting values {}, {}", value_container, max );\
...
#endif // MAGNEFU_MATH_OVERFLOW_CHECK
...
// Ceil
    ...
    {
        MF_CORE_ASSERT(min < max, "get_random_value() | min > max");
