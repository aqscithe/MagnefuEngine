...
namespace Magnefu
{
    ...
    // Control byte --------------------------------------------------------------- //
    // Following Google's abseil library convention - based on performance.
    ...
    inline void FlatHashMap<K, V>::shutdown() {
        mffree(control_bytes, allocator);
    }
    ...
    void FlatHashMap<K, V>::resize(u64 new_capacity) {
        ...
        for (size_t i = 0; i != old_capacity; ++i) {
            if (control_is_full(old_control_bytes[i])) {
                ...
                Magnefu::memoryCopy(slots_ + new_i, old_slots + i, sizeof(KeyValue));
            }
        }

        if (old_capacity) {
            mffree(old_control_bytes, allocator);
        }
    }
